#!/usr/bin/env python3
"""Dashboard web Smart Frigo — Flask + MQTT + SSE."""
...
from mqtt_bridge import FrigoMqttBridge

app = Flask(__name__)
bridge: FrigoMqttBridge | None = None
...
def parse_args() -> argparse.Namespace:
  p = argparse.ArgumentParser(description="Dashboard Smart Frigo El Jezi")
  p.add_argument("--broker", default="localhost")
  p.add_argument("--port", type=int, default=1883)
  p.add_argument("--host", default="127.0.0.1")
  p.add_argument("--web-port", type=int, default=5090)
  return p.parse_args()
...
def main() -> int:
  global bridge
  args = parse_args()
  bridge = FrigoMqttBridge(broker=args.broker, port=args.port)
  bridge.start()
  print(f"Smart Frigo dashboard : http://{args.host}:{args.web_port}")
  try:
