#!/usr/bin/env python3
"""Dashboard web Smart Farm — Flask + MQTT + SSE."""
...
from mqtt_bridge import FarmMqttBridge

app = Flask(__name__)
bridge: FarmMqttBridge | None = None
...
def parse_args() -> argparse.Namespace:
  p = argparse.ArgumentParser(description="Dashboard Smart Farm El Jezi")
...
def main() -> int:
  global bridge
  args = parse_args()
  bridge = FarmMqttBridge(broker=args.broker, port=args.port)
  bridge.start()
  print(f"Smart Farm dashboard : http://{args.host}:{args.web_port}")
  try:
