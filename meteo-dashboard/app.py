#!/usr/bin/env python3
"""Dashboard web Smart Meteo — Flask + MQTT + SSE."""
...
from mqtt_bridge import MeteoMqttBridge

app = Flask(__name__)
bridge: MeteoMqttBridge | None = None
...
def parse_args() -> argparse.Namespace:
  p = argparse.ArgumentParser(description="Dashboard Smart Meteo El Jezi")
  p.add_argument("--broker", default="localhost")
  p.add_argument("--port", type=int, default=1883)
  p.add_argument("--host", default="127.0.0.1")
  p.add_argument("--web-port", type=int, default=5080)
  return p.parse_args()
...
def main() -> int:
  global bridge
  args = parse_args()
  bridge = MeteoMqttBridge(broker=args.broker, port=args.port)
  bridge.start()
  print(f"Smart Meteo dashboard : http://{args.host}:{args.web_port}")
  try:
